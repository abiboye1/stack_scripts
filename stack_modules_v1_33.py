...
#Creating Cross Region Bucket Replication
def cross_region_replication(**kwargs):
	try:
...
#Creating Cross Account Bucket Replication
def cross_acct_replication(**kwargs):
	try:
		#Calling create STS function
		s3_client = create_sts()
...
def upload_s3_bucket(**kwargs):
	...
	folder = "/home/oracle/scripts/practicedir_abi_sep23/%s"%(kwargs['directory_name'])
	files = os.listdir(folder)

	#Looping through the folder to fetch files for upload	
	for doc in files:
		"""
		#bucket_response = s3_client.bucket[bucket_name].put_object(Key=doc)
		bucket_response = s3_client.Object(bucket_name, doc).upload_file(os.path.join(folder, doc))
		if bucket_response['ResponseMetadata']['HTTPStatusCode'] == 200:
			print("%s was uploaded to %s successfully"%(doc, bucket_name))
		"""
		with open(os.path.join(folder, doc), 'rb') as f:
            # Use put_object to upload file content
			s3_client.put_object(Bucket=bucket_name, Key=doc, Body=f)
			print("%s was uploaded to %s successfully"%(doc, bucket_name))			
