...
EOF
	...
	#Creating a par file 
	echo "userid='/ as sysdba'">export_abib.par
	echo "schemas=${par_schema}">>export_abib.par
	echo "dumpfile=${par_schema}_abib_${TS}.dmp">>export_abib.par
	echo "logfile=${par_schema}_abib_${TS}.log">> export_abib.par
	echo "directory=${par_directory}">>export_abib.par
	expdp parfile=export_abib.par
	...
	if ( grep "successfully completed" /backup/AWSSEP23/APEXDB/${par_schema}_abib_${TS}.log ); then
	...
	case $FUNCTION in
		...
		database_backup)
			if [[ $FUNCTION == "database_backup" ]]; then
				...
				database_backup $par_schema $par_directory
			fi
		;;
		backup_f_d)
