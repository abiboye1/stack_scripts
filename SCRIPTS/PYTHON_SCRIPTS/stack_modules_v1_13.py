   ...
   #Database backup function variable declaration
	...
	try:
      ...
      #Running expdp command to initiate database export
		...
		"""
		if "completed with" not in export_result.lower():
			raise Exception("Export failed! {}".format(export_result))
		
		# Check the log file for a successful completion message
		log_file_path = '{}/{}_{}_{}.log'.format(practicedir, schemas, RUNNER, TS)
		with open(log_file_path, 'r') as log_file:
			log_content = log_file.read()
			if "successfully completed" not in log_content.lower():
				raise Exception("Export failed! Log content: {}".format(log_content))
		"""
	except Exception as e:
		print("Export failed! Error: {}".format(str(e)))
...
#def database_import(practicedir, backup_location, RUNNER, schemas, datapump_dir, import_DB):
def DATA_MIGRATION(practicedir, backup_location, RUNNER, schemas, datapump_dir, import_DB):	
	...
	dumpfile = '{}/{}_{}_{}.dmp'.format(backup_location, schemas, RUNNER, TS)
	...
	G_Zipp(source_path=dumpfile, output_gzip=gzipped_dumpfile)


	# Calling the unzip function to unzip the dumpfile
	#unzipped_dumpfile = '{}.unzip'.format(gzipped_dumpfile)
	unzipped_dumpfile = gzipped_dumpfile.replace('.gzip', '')
	Unzipp(source_path=gzipped_dumpfile, output_f_d=unzipped_dumpfile, zip_type='f')
	# Copying unzipped dumpfile to SAMD directory
	sh.copy(unzipped_dumpfile, '/backup/AWSSEP23/SAMD')
	new_dumpfile_path = os.path.join('/backup/AWSSEP23/SAMD', unzipped_dumpfile)
	...
	with open('impdp_%s_%s.par'%(schemas, RUNNER), 'w+') as pf:
		pf.write('userid="/ as sysdba"\n''schemas={x}\n''remap_schema={x}:{x}_{y}_{z}\n''dumpfile={dmp}\n''logfile=impdp_{x}_{y}.log\n''directory={a}\n''table_exists_action=replace'.format(x=schemas, y=RUNNER, z=TS, a=datapump_dir, dmp=new_dumpfile))
