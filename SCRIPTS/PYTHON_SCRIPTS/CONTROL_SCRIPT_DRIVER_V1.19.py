...
import stack_modules_v1_19 as sm, sys
...
#				sm.backup_function(source=sys.argv[2], destination=sys.argv[3], OP_ID=sys.argv[4], OP_NAME=sys.argv[5], STATUS=sys.argv[6], OP_TYPE=sys.argv[7], RUNNER=sys.argv[8])
	...
	#Checking decision for Data Migration function call
   	...
   	#Checking if only one argument is passed and taking inputs
		sm.aws_create_users(service="iam", user="stack_usr_aut")
		...
		if len(sys.argv) - 1 == 1:
			...
			sm.aws_create_group(service=sys.argv[2], group=sys.argv[3], policy_arn=sys.argv[4])
       ...
       #Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			...
			sm.aws_add_user_to_group(service=sys.argv[2], group=sys.argv[3], user=sys.argv[4], password=sys.argv[5])
