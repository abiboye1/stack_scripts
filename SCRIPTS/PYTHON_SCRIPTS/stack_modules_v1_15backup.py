...
def db_connection(**kwargs):
	if kwargs['STATUS'] == 'Running':
		...
		OP_NAME_INS = kwargs['OP_NAME'],
		OP_STARTTIME_INS = OP_STARTTIME,
		OP_ENDTIME_INS = OP_ENDTIME,
		RUNNER_INS = kwargs['RUNNER'],
		STATUS_INS = kwargs['STATUS'],
		OP_TYPE_INS = kwargs['OP_TYPE'])
   	# To commit to the database, i.e. ensure that there is no rollback
		connection.commit()
		cursor.close()
		connection.close()
		...
		connection = cx.connect(user='STACK_ABI_SEP23', password='stackinc', dsn='MKIT-DEV-OEM/APEXDB')
		cursor = connection.cursor()
		cursor.execute("""update operations set OP_ENDTIME = :OP_ENDTIME_INS where OP_ID = :OP_ID_INS""", OP_ENDTIME_INS = OP_ENDTIME, OP_ID_INS = kwargs['OP_ID'])
		cursor.execute("""update operations set STATUS = :STATUS_INS where OP_ID = :OP_ID_INS""", STATUS_INS = STATUS, OP_ID_INS = kwargs['OP_ID'])
		connection.commit()
		cursor.close()
		connection.close()
