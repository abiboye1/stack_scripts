...
def G_Zipp(**kwargs):
	output_gzip_TS = '{}_{}'.format(kwargs['output_gzip'], TS)
	...
	if os.path.isfile(kwargs['source_path']):
		with open(kwargs['source_path'], 'rb') as my_input:
			with gzip.open(output_gzip_TS, 'wb') as my_output: ##### Changes
				sh.copyfileobj(my_input, my_output)
				print("Successfully zipped file %s to %s"%(kwargs['source_path'], output_gzip_TS))
		...
		with open(tar_filename, 'rb') as my_input_tar_file:
			...
			with gzip.open('%s'%output_gzip_TS, 'wb') as my_output_gzip_file:  ### Changes
				sh.copyfileobj(my_input_tar_file, my_output_gzip_file)
				print("Successfully zipped directory %s to %s"%(kwargs['source_path'], output_gzip_TS))
...
def disk_maintenance_check_on_prem(**kwargs):
	...
	if disk_utilization > float(kwargs['threshold']) or disk_utilization == 100:
		print("Warning!!! Disk utilization is at {}%, over {}% threshold".format(disk_utilization, kwargs['threshold']))
	else:
...
def database_import(**kwargs):
	...
	#Unzipp(kwargs['source_path'], kwargs['output_f_d'], 'f')
	Unzipp(zip_type='f', **kwargs)
	# Copying unzipped dumpfile to SAMD directory
	sh.copy('/backup/AWSSEP23/APEXDB/{}'.format(kwargs['output_f_d']), '/backup/AWSSEP23/SAMD')
	...
	with open('impdp_%s_%s.par'%(kwargs['schemas'], kwargs['RUNNER']), 'w+') as pf:
		...
		pf.write('userid="/ as sysdba"\n''schemas={x}\n''remap_schema={x}:{x}_{y}_{z}\n''dumpfile={dumpfile}\n''logfile=impdp_{x}_{y}.log\n''directory={a}\n''table_exists_action=replace'.format(x=kwargs['schemas'], y=kwargs['RUNNER'], z=TS, a=kwargs['datapump_dir'], dumpfile=kwargs['output_f_d']))
