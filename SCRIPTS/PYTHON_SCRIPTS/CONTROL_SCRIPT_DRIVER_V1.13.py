...
import stack_modules_v1_13 as sm, sys
	...
	#Checking decision for Import function call
   	...
   	#Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			...
			#practicedir= input("Enter the practicedir path: ")
			...
			try:
				#sm.database_import(schemas, RUNNER, datapump_dir, import_DB, practicedir)
				sm.DATA_MIGRATION(schemas, RUNNER, datapump_dir, import_DB, practicedir)
				sm.STACK_EMAIL(TO_EMAIL, SUBJECT, BODY)
			except:
      ...
      #Checking if the right number of command line argument is passed		
			...
			BODY = "Import successful!!!\n{} schema was successfully imported to {}".format(schemas, import_DB)
			#try: 	
				#sm.database_import(practicedir, backup_location, RUNNER, schemas, datapump_dir, import_DB)
			sm.DATA_MIGRATION(practicedir, backup_location, RUNNER, schemas, datapump_dir, import_DB)
			sm.STACK_EMAIL(TO_EMAIL, SUBJECT, BODY)
			#except:
			#	BODY = "Import failed!!!\n{} schema import to {} failed".format(schemas, import_DB)
			#	sm.STACK_EMAIL(TO_EMAIL, SUBJECT, BODY)
