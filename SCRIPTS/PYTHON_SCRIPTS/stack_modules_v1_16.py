...
import sys, shutil as sh, time as t, os, psutil, gzip, pathlib as pl, tarfile, smtplib as s, cx_Oracle as cx, subprocess as sp, datetime as dt
...
#		cursor.execute("""update PROD_ACTIVITIES set OP_ENDTIME = :OP_ENDTIME_INS, STATUS = :STATUS_INS where OP_NAME = :OP_NAME_INS""", OP_NAME_INS = op_name, OP_ENDTIME_INS = OP_ENDTIME, STATUS_INS = STATUS)		

		#Using a "SELECT...JOIN" to print out an ouput
		cursor.execute("""select a.OP_NAME, a.OP_TYPE, b.OP_STARTTIME, b.OP_ENDTIME, b.MON_EMAIL from PROD_OPERATIONS A FULL join PROD_ACTIVITIES B on a.OP_ID=b.OP_ID where a.OP_NAME = :OP_NAME_INS""",OP_NAME_INS = op_name)
