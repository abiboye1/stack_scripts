...
import stack_modules_v1_12 as sm, sys
...
def stack_modules_function():
	...
	#Checking decision for Import function call
      ...
      #Checking if the right number of command line argument is passed		
			...
			try: 	
				sm.database_import(source_path=sys.argv[2], output_f_d=sys.argv[3], schemas=sys.argv[4], RUNNER=sys.argv[5], datapump_dir=sys.argv[6], import_DB=sys.argv[7], practicedir=sys.argv[8])
				sm.STACK_EMAIL(TO_EMAIL=sys.argv[9], SUBJECT=sys.argv[10], BODY="Import successful!!!")
			except:
				#BODY = "Import failed!!!\n{} schema import to {} failed".format(schemas, import_DB)
