...
def delete_users(**kwargs):
	#Calling database loggin function
	db_connection(**kwargs)	

	iam = boto3.client(kwargs['service'])
	#Deleting user login profile
	try:
		response1 = iam.delete_login_profile(UserName=kwargs['user'])
		if response1['ResponseMetadata']['HTTPStatusCode'] == 200:
			print("Login profile successfully deleted")
	...
	if kwargs['access_key']:
		try:
			response2 = iam.delete_access_key(UserName=kwargs['user'], AccessKeyId=kwargs['access_key'])
	...
	if 'group' in kwargs:
		try:
			response3 = iam.remove_user_from_group(GroupName=kwargs['group'], UserName=kwargs['user'])
			if response3['ResponseMetadata']['HTTPStatusCode'] == 200:
				print("{} removed from {} group successfully".format(kwargs['user'], kwargs['group']))
		except ClientError as error:
			print(error.response)
			if error.response['Error']['Code'] == "NoSuchEntity":
				pass

	#Deactivating MFA device
	if kwargs['serial_num']:
		try:
			response4 = iam.deactivate_mfa_device(UserName=kwargs['user'], SerialNumber=kwargs['serial_num'])
			if response4['ResponseMetadata']['HTTPStatusCode'] == 200:
				print("MFA device deactivated successfully")
		except ClientError as error:
			print(error.response)
			if error.response['Error']['Code'] == "NoSuchEntity":
				pass

	#Delete user policy (Inline policy)
	if kwargs['policy_name']:
		try:
			response5 = iam.delete_user_policy(UserName=kwargs['user'], PolicyName=kwargs['policy_name'])
			if response5['ResponseMetadata']['HTTPStatusCode'] == 200:
				print("User / Inline policy deleted successfully")
		except ClientError as error:
			print(error.response)
			if error.response['Error']['Code'] == "NoSuchEntity":
				pass
	...
	#Delete IAM user 
	try:
		response = iam.delete_user(UserName=(kwargs['user']))
		if response['ResponseMetadata']['HTTPStatusCode'] == 200:
			print("{} deleted successfully".format(kwargs['user']))
			#Calling db_conections function for completed status
			kwargs['STATUS']='Completed'
			db_connection(**kwargs)
	except ClientError as error:
		print(error.response)
		if error.response['Error']['Code'] == "NoSuchEntity":
			print("%s does not exist"%(kwargs['user']))
