...
import stack_modules_v1_33 as sm, sys
...
def stack_modules_function():
	...
	# 18 --> Set Versioning
	# 19 --> Set Encryption
	# 20 --> Server Access Log
	# 21 --> Object Level Log
	# 22 --> Static Web Host
	# 23 --> Cross Account Replication
...
#				sm.backup_function(source=sys.argv[2], destination=sys.argv[3], OP_ID=sys.argv[4], OP_NAME=sys.argv[5], STATUS=sys.argv[6], OP_TYPE=sys.argv[7], RUNNER=sys.argv[8])
	...
	#Checking decision for Data Migration function call
       ...
       #Checking if only one argument is passed and taking inputs
		...
		# CALLING UPLOAD CONTENT FUNCTION
	elif decision_no == "15": # UPLOAD CONTENT
		#Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			sm.upload_s3_bucket(bucket=input("Enter bucket name: "), directory_name=input("Enter directory name: "))
		elif len(sys.argv) - 1 != 3:
			print("Error!\nThere are {} arguments in this call".format(len(sys.argv) - 1))
			print("\nThis function call requires 2 arguments and should be ran like this:\npython CONTROL_SCRIPT_DRIVER _V1.24.py 15 bucket_name directory_name")
		elif len(sys.argv) - 1 == 3:
			sm.upload_s3_bucket(bucket=sys.argv[2], directory_name=sys.argv[3])

	
	   # CALLING EMPTY BUCKET CONTENT FUNCTION
	elif decision_no == "16": # EMPTY BUCKET CONTENT
   	#Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			sm.empty_bucket_content(bucket=input("Enter bucket name: "), object1=input("Enter object1: "), object2=input("Enter object2: "), object3=input("Enter object3: "), object4=input("Enter object4: "), object5=input("Enter object5: "), object6=input("Enter object6: "))
		elif len(sys.argv) - 1 != 5:
			print("Error!\nThere are {} arguments in this call".format(len(sys.argv) - 1))
			print("\nThis function call requires 2 arguments and should be ran like this:\npython CONTROL_SCRIPT_DRIVER _V1.24.py 16 bucket_name object1 object2 ...")
		elif len(sys.argv) - 1 == 5:
			sm.empty_bucket_content(bucket=sys.argv[2], object1=sys.argv[3], object2=sys.argv[4], object3=sys.argv[5])


		# CALLING LIST BUCKET CONTENT FUNCTION
	elif decision_no == "17": # LIST BUCKET CONTENT
      #Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			sm.list_bucket_content(bucket=input("Enter bucket name: "))
		elif len(sys.argv) - 1 != 2:
			print("Error!\nThere are {} arguments in this call".format(len(sys.argv) - 1))
			print("\nThis function call requires 2 arguments and should be ran like this:\npython CONTROL_SCRIPT_DRIVER _V1.26.py 17 bucket_name")
		elif len(sys.argv) - 1 == 2:
			sm.list_bucket_content(bucket=sys.argv[2])


	   # CALLING SET BUCKET VERSIONING FUNCTION
	elif decision_no == "18": # SET VERSIONING 
   	#Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			sm.set_versioning(bucket=input("Enter bucket name: "), status=input("Enter status :"))
		elif len(sys.argv) - 1 != 3:
			print("Error!\nThere are {} arguments in this call".format(len(sys.argv) - 1))
			print("\nThis function call requires 3 arguments and should be ran like this:\npython CONTROL_SCRIPT_DRIVER _V1.27.py 17 bucket_name status")
		elif len(sys.argv) - 1 == 3:
			sm.set_versioning(bucket=sys.argv[2], status=sys.argv[3])


   	# CALLING SET ENCRYPTION FUNCTION
	elif decision_no == "19": # SET ENCRYPTION
   	#Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			sm.set_encryption(bucket=input("Enter bucket name: "), status=input("Enter status :"), kms_key=input("Enter KMS key: "))
		elif len(sys.argv) - 1 != 4:
			print("Error!\nThere are {} arguments in this call".format(len(sys.argv) - 1))
			print("\nThis function call requires 4 arguments and should be ran like this:\npython CONTROL_SCRIPT_DRIVER _V1.27.py 17 bucket_name status")
		elif len(sys.argv) - 1 == 4:
			sm.set_encryption(bucket=sys.argv[2], status=sys.argv[3], kms_key=sys.argv[4])


	    # CALLING SERVER ACCESS LOGGING FUNCTION
	elif decision_no == "20": # SERVER ACCESS LOG
		#Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			sm.server_access_log(bucket=input("Enter bucket name: "), target_bucket=input("Enter target bucket :"))
		elif len(sys.argv) - 1 != 3:
			print("Error!\nThere are {} arguments in this call".format(len(sys.argv) - 1))
			print("\nThis function call requires 3 arguments and should be ran like this:\npython CONTROL_SCRIPT_DRIVER _V1.29.py 20 bucket_name target_bucket")
		elif len(sys.argv) - 1 == 3:
			sm.server_access_log(bucket=sys.argv[2], target_bucket=sys.argv[3])


     # CALLING OBJECT LEVEL LOGGING FUNCTION
	elif decision_no == "21": # OBJECT LEVEL LOG
       #Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			sm.object_level_log(bucket=input("Enter bucket name: "), trail_name=input("Enter trail name :"))
		elif len(sys.argv) - 1 != 3:
			print("Error!\nThere are {} arguments in this call".format(len(sys.argv) - 1))
			print("\nThis function call requires 3 arguments and should be ran like this:\npython CONTROL_SCRIPT_DRIVER _V1.29.py 20 bucket_name target_bucket")
		elif len(sys.argv) - 1 == 3:
			sm.object_level_log(bucket=sys.argv[2], trail_name=sys.argv[3])


   # CALLING STATIC WEB HOSTING FUNCTION
	elif decision_no == "22": # STATIC WEB HOST
        #Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			sm.static_web_host(bucket=input("Enter bucket name: "), error=input("Enter error document name :"), index=input("Enter index document name :"))
		elif len(sys.argv) - 1 != 4:
			print("Error!\nThere are {} arguments in this call".format(len(sys.argv) - 1))
			print("\nThis function call requires 3 arguments and should be ran like this:\npython CONTROL_SCRIPT_DRIVER _V1.29.py 20 bucket_name target_bucket")
		elif len(sys.argv) - 1 == 4:
			sm.static_web_host(bucket=sys.argv[2], error=sys.argv[3], index=sys.argv[4])


	# CALLING CROSS ACCOUNT APPLICATION FUNCTION
	elif decision_no == "23": # CROSS ACCOUNT APPLICATION
   	#Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			sm.cross_acct_replication(bucket=input("Enter src bucket name: "), role_arn=input("Enter src acct role arn :"), dest_bucket=input("Enter dest bucket name :"))
		elif len(sys.argv) - 1 != 4:
			print("Error!\nThere are {} arguments in this call".format(len(sys.argv) - 1))
			print("\nThis function call requires 4 arguments and should be ran like this:\npython CONTROL_SCRIPT_DRIVER _V1.32.py 23 bucket_name role_arn target_bucket")
		elif len(sys.argv) - 1 == 4:
			sm.cross_acct_replication(bucket=sys.argv[2], role_arn=sys.argv[3], dest_bucket=sys.argv[4])


	# CALLING CROSS REGION APPLICATION FUNCTION
	elif decision_no == "24": # CROSS REGION APPLICATION
   #Checking if only one argument is passed and taking inputs
		if len(sys.argv) - 1 == 1:
			sm.cross_region_replication(bucket=input("Enter src bucket name: "), role_arn=input("Enter src acct role arn :"), dest_bucket_arn=input("Enter dest bucket name :"))
		elif len(sys.argv) - 1 != 4:
			print("Error!\nThere are {} arguments in this call".format(len(sys.argv) - 1))
			print("\nThis function call requires 4 arguments and should be ran like this:\npython CONTROL_SCRIPT_DRIVER _V1.33.py 24 bucket_name role_arn target_bucket_arn")
		elif len(sys.argv) - 1 == 4:
			sm.cross_region_replication(bucket=sys.argv[2], role_arn=sys.argv[3], dest_bucket_arn=sys.argv[4])
